...
/**
 * maps a bool value to the according field
...
class BoolType implements TypeInterface {
    ...
    public static function execute(object &$model, array $data, array $field) : void 
    {
        ...
        if (isset($data[$field[$fieldName]['api']['name']]) && $data[$field[$fieldName]['api']['name']] !== null && $data[$field[$fieldName]['api']['name']] !== '') {
            $model->set($fieldName, boolval($data[$field[$fieldName]['api']['name']]));
        } else {
